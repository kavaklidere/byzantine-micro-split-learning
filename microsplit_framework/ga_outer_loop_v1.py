...
class GAConfig:
    ...
    T_provisional: int = 15   # max candidates evaluated with real inference per blueprint
...
def _generate_blueprint(
    partition_ids: list[str], device_ids: list[str], budget: int
) -> DeviceBlueprint:
    extras: dict[tuple[str, str], int] = {}
    for _ in range(budget):
        key = (random.choice(partition_ids), random.choice(device_ids))
...
def _crossover(
    ...
    pa_assigns = pa.all_assignments()
    pb_assigns = pb.all_assignments()
    all_keys = sorted(set(pa_assigns.keys()) | set(pb_assigns.keys()))
    extras = {k: random.choice([pa_assigns.get(k, 0), pb_assigns.get(k, 0)]) for k in all_keys}
    total = sum(extras.values())
    while total > budget:
        donors = sorted(k for k, v in extras.items() if v > 0)
        chosen = random.choice(donors)
        extras[chosen] -= 1
        total -= 1
    while total < budget:
        key = (random.choice(partition_ids), random.choice(device_ids))
...
def _mutate(
    ...
    if random.random() >= mutation_rate:
        return blueprint
    extras = dict(blueprint.all_assignments())
    donors = sorted(k for k, v in extras.items() if v > 0)
    if not donors:
        return blueprint
    donor_key = random.choice(donors)
    extras[donor_key] -= 1
    if extras[donor_key] == 0:
        del extras[donor_key]
    new_key = (random.choice(partition_ids), random.choice(device_ids))
    extras[new_key] = extras.get(new_key, 0) + 1
    return DeviceBlueprint.from_dict(extras)
...
def run_ga(
    ...
    seed: int = 42,
    seed_blueprints: list[DeviceBlueprint] | None = None,
) -> GAResult:
    """
    Minimax GA: maximise worst-case Top-1 accuracy under m_attackers simultaneously
    compromised devices, over all DeviceBlueprints with exactly `budget` total extra
    replicas assigned across (partition, device) pairs.

    potentials must be pre-computed via compute_attack_potentials() before calling run_ga.
    ...
    fitness_cache: dict[DeviceBlueprint, tuple[float, float, float, frozenset]] = {}

    def get_fitness(bp: DeviceBlueprint) -> tuple[float, float, float, frozenset]:
    ...
    population += [_generate_blueprint(partition_ids, device_ids, config.budget) for _ in range(config.population_size - len(population))]
    ...
    for gen in range(config.generations):
        ...
        worst_attacker_str = " + ".join(sorted(gen_best_worst_combo))
        history.append({
            "generation":     gen + 1,
        ...
        print(
            ...
            f"Avg (top-T): {gen_best_best_bp_avg_top1:.2f}% | "
        ...
        while len(new_population) < config.population_size:
            ...
            child = _crossover(parent1, parent2, partition_ids, device_ids, config.budget)
            child = _mutate(child, partition_ids, device_ids, config.mutation_rate)
